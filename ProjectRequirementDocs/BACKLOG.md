...
22. ~~Level Complete / Level Failed reskin.~~ Done. Icon-in-circle headers (teal anchor for complete, orange hourglass for failed), two-tone headings, stat boxes with live values, Continue/Retry buttons matching Main Menu button styling, Back to Menu button on the fail screen (visual only, navigation not yet wired). HUD bleed-through fixed (item 32, closed). Text softness at small Editor viewport sizes investigated and partially mitigated (font sizes bumped), root cause understood (legacy UI.Text has no mipmap chain, softens below a Canvas scale factor of 1.0). Verified up to a 472x851 Editor window, still slightly soft at that size on the project owner's monitor. Real device testing needed to confirm whether this is a non-issue in practice (expected, since phone resolutions typically exceed the 941x1672 reference) or still needs the TextMeshPro migration noted in item 33. Revisit once a real device or Unity Cloud Build test is possible.
23. ~~Settings screen reskin.~~ Done. Hexagonal outline back button, anchor+heading treatment, diamond divider, AUDIO panel matching the Layout and Spacing System spec exactly (measured and verified), lighthouse background with a dark overlay for legibility, three panel-adjacent buttons (Back to Menu, Reset Progress, Quit) resized to 828px matching the panel above them, all using the shared ButtonPrimaryTeal/ButtonSecondarySlate prefabs.
...
25. ~~Pause button and pause/resume flow.~~ Done. Hex-frame pause button on the gameplay HUD, freezes the timer and input, shows a Pause panel matching the established panel spec with an inline mini Settings (Sound Effects/Music toggles, same SoundSettings backing as the Settings scene, no scene navigation so gameplay state is preserved), Resume and Back to Menu buttons using the shared panel-adjacent button prefabs. HUD bleed-through behind the Pause panel found and fixed using the same explicit hide/show mechanism as Level Complete/Failed.
26. ~~Hint system.~~ Done. Hex-frame hint button bottom-right of the gameplay HUD, mirroring the pause button. Hint allowance is level-based, resetting fresh each level: 3 hints for levels 1-3, 2 for levels 4-6, 1 for levels 7-9, 0 for level 10+. Using a hint reveals one random unmatched pair for 1.5 seconds without counting as a move, reuses the existing inputLocked mechanism, correctly blocked during card resolution and correctly interacts with pause. At 0 hints the button stays visible but disabled and dimmed (0.35 alpha) rather than hidden.
...
29. ~~Separate Music and Sound Effects toggles.~~ Done, completed alongside the Settings reskin. SoundSettings.cs now exposes independent SfxEnabled and MusicEnabled booleans, both persisted separately, both wired into the reskinned AUDIO panel's two toggles. Music toggle has no audio asset to control yet, deferred, but the toggle and its persistence are fully functional and ready.
...
32. ~~Gameplay HUD bleeds through behind Level Complete / Level Failed panels.~~ Done. Fixed by grouping all HUD elements under a GameplayHUD container, explicitly hidden during end panel states and restored on Retry/Continue, rather than relying on draw order alone. Verified with zero bleed-through on both Level Complete and Level Failed, HUD correctly restores with fresh values after Retry/Continue. Note: the same class of bug recurred on the Pause panel (item 25) since it was built separately and did not reuse this exact hide/restore mechanism, tracked and fixed under item 25's own testing.

### F. Deferred, not urgent right now
33. **Optional future improvement: migrate from legacy UI.Text to TextMeshPro.** Legacy UI.Text has no mipmap chain for its dynamic font atlas, causing visible softness at Canvas scale factors below 1.0. Confirmed still slightly soft even at a 472x851 Unity Editor test window, below the reference resolution. Font sizes were bumped on the Level Complete/Failed panels as a practical mitigation. Real device or Unity Cloud Build testing is needed to confirm whether this is actually a non-issue in production (expected, since real phone resolutions typically exceed the 941x1672 reference) before deciding if the fuller TextMeshPro migration is worth doing. Revisit once real device testing is possible, tied to Stage 8 (iOS build pipeline) below.
